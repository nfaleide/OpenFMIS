...
def _fernet() -> Fernet:
    """Derive a Fernet key from the JWT secret (deterministic, 32-byte key)."""
...
class EmailDeliveryService:
    ...
    async def set_config(
        ...
        if smtp_password is not None:
            config.smtp_password_encrypted = _encrypt_password(smtp_password)
    ...
    async def _send_smtp(
        ...
        try:
            ...
            await aiosmtplib.send(
                ...
                password=(
                    _decrypt_password(config.smtp_password_encrypted)
